<a onclick="noty_activateSub(event, {{ $entry }})" class="btn btn-sm btn-link" href="#"
    data-toggle="tooltip" title="Activate Project"><i class="la la-play"></i> Activate</a>
<style>
    .input-label {
        display: block;
        margin-bottom: 5px;
...
<script>
    function noty_activateSub(event, entry) {
        ...
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                if ((!input1 && !input2) || (input1 && input2)) {
                    ...
                    if (input1) {
                        if (Number(input1) < 1 || Number(input1) > 24) {
                            ...
                            if (hour == 0) {
                                ...
                                Swal.fire({
                                    title: "Activating The Subservice Until " + input1.toString() +
                                        ":00 " +
                                        day +
                                        " (" + hour + " Hours)",
                        ...
                        hour = Number(input2);
                        Swal.fire({
                            title: "Activating The Subservice For " + hour.toString() + " Hours",
                            showConfirmButton: false,
                            allowOutsideClick: false,
                            allowEscapeKey: false,
                            icon: 'info',
                    ...
                    $.ajax({
                        type: "GET",
                        url: "/admin/subservice/activate/" + name,
                        data: {
                            hour: hour,
                        },
                        success: function(response) {
                            ...
                            if (response == 'success') {
                                Swal.fire({
                                    title: "Succes!",
                                    text: "The Subservice Is Activated For " + hour
                                        .toString() + " Hours!",
                                    icon: "success"
                                }).then(function() {
                                    location.reload();
